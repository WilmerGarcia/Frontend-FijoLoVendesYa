...
export function NewPassword(props) {
  ...
  const [ setError] = useState(null);
    ...
    const {pass} = values;
     ...
     const response = await axios
       .post("http://localhost:4000/api/tienda/actualizarpass", {pass: pass})
       .then(response => {swal({
         title: "EXITOSO",
  ...
  //Inicializa los valores del formulario, onsubmit envia la informacion al useFormik y se validan los campos con el validationSchema
    const formik = useFormik({initialValues :{pass:""}, validateOnBlur: true, onSubmit, validationSchema: validationSchema});
